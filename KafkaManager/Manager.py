#!/usr/bin/env python
from SubscribeCorePublishSystem import SubscribeCorePublishSystem
from SubscribeSystemPublishCore import SubscribeSystemPublishCore
...
class Manager:
    ...
    def subToSystemPublishToCore(self, systemTopic, coreTopic):
        systemSub = SubscribeSystemPublishCore(
                        self.systemName, self.systemBrokerIP, self.systemBrokerPort, systemTopic,
                        self.coreBrokerIP, self.coreBrokerPort, coreTopic, self.logEnabled, self.logOnlyAtTheEnd, self.logBrokerIP, self.logBrokerPort)

        self.fromSystemToCore.append(systemSub)

    def subToCorePublishToSystem(self, coreTopic, systemTopic):
        coreSub = SubscribeCorePublishSystem(
                        self.systemName, self.coreBrokerIP, self.coreBrokerPort, coreTopic,
                        self.systemBrokerIP, self.systemBrokerPort, systemTopic, self.logEnabled, self.logOnlyAtTheEnd, self.logBrokerIP, self.logBrokerPort)
